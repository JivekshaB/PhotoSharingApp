...
//        mNumResources = media.size();
     ...
     /*   int indices = mNumResources;
        ...
        if (mNumResources > 0) {
            for (int k = 0; k < mNumResources; k++) {
                int numMediaIndices = mUserStories.getJSONObject(k).getJSONArray(mContext.getString(R.string.user_stories)).length();
                for (int i = 0; i < numMediaIndices; i++) {
                    try {
//          int numMediaIndices = mMedia.get(i).getMedia().size();
        ...
        if (mUserStories.getJSONObject(0).getJSONArray(mContext.getString(R.string.user_stories)) != null) {
            mResources.add(0, mUserStories.getJSONObject(0).getJSONArray(mContext.getString(R.string.user_stories)));
...
//            e.printStackTrace();
        ...
        try {
            ...
            final int surface2ResourceIndex = 1;
...
//        rotateToStartingIndex();
    ...
    public MediaSource buildMediaSource(Uri uri) {
        ...
        DataSource.Factory dataSourceFactory = new DefaultDataSourceFactory(mContext,
                Util.getUserAgent(mContext, "InstaApp"), null);
